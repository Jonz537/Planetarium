...
public class Main {
    ...
    public static void init(StarSystem starSystem) {
        try {
            ...
            starSystem.getPlanetByName("Giove").addMoon(new Moon("Orecchie", 1, new Position(5, 1), "Giove", starSystem), starSystem);
            starSystem.getPlanetByName("Giove").addMoon(new Moon("Gatto", 1, new Position(5, 4), "Giove", starSystem), starSystem);
            starSystem.addPlanet(new Planet("Dante", 1, new Position(10, 0), starSystem));
            starSystem.getPlanetByName("Dante").addMoon(new Moon("Nero", 1, new Position(10, 0.9), "Dante", starSystem), starSystem);
            starSystem.getPlanetByName("Dante").addMoon(new Moon("V", 1, new Position(10, 2), "Dante", starSystem), starSystem);
    ...
    ○ Data una luna, visualizzazione del percorso [stella > pianeta > luna] necessario per raggiungerla. FATTO
    ● Calcolo del centro di massa su richiesta, sulla base delle informazioni disponibili volta per volta. FATTO

    FATTO Calcolo della rotta fra due corpi celesti:
        ○ Su richiesta dell’utente, il programma deve mostrare la rotta fra due corpi del sistema stellare
        selezionati dall’utente stesso.
        ○ Le leggi imperiali impongono che, nel caso in cui si viaggi fra due corpi di uguale “grado” (due lune, o
        due pianeti), si faccia scalo sul corpo celeste di grado più alto (rispettivamente, il pianeta in comune o la
        stella del sistema). In questo modo, la rotta fra due corpi celesti qualunque sia unica, e l’Impero riesce a
        mantenere l’ordine.
        ○ La rotta deve essere rappresentata come sequenza di corpi celesti (ad esempio: Luna 1 > Pianeta 1 >
        Stella > Pianeta 2).
        ○ Deve essere indicata la distanza totale che si percorrerebbe seguendo la sequenza sopra indicata
        (nell’esempio precedente: 8,65).
    FATTO  Calcolo della collisione fra i corpi celesti:
        ○ Su richiesta dell’utente, il programma deve stabilire se sia possibile che due corpi qualunque del
        sistema collidano uno contro l’altro. Non è necessario stabilire quando e dove, né quali corpi
        siano, ma solamente se questo possa succedere.
        ○ Due corpi possono collidere se e solo se esiste una configurazione del sistema per cui due corpi
        possono trovarsi nella stessa posizione puntuale.
        ○ Ogni corpo ha distanza fissa dal corpo attorno a cui ruota (detta raggio di rivoluzione). Pertanto,
        nessuna luna colliderà mai col proprio pianeta e nessun pianeta colliderà mai con la stella del
        sistema.
     TODO testing*/
